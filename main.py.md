...
def main_menu(name):  
    ...
    btn_recovery = types.InlineKeyboardButton("В разроботке", url="[https://www.python.org/downloads/](https://www.python.org/downloads/)")  
    ...
    try:  
        if os.path.exists(photo_path):  
            with open(photo_path, 'rb') as photo:  
                bot.send_photo([message.chat.id](http://message.chat.id), photo, caption=text, reply_markup=markup, parse_mode="HTML")  
        else:  
            bot.send_message([message.chat.id](http://message.chat.id), text, reply_markup=markup, parse_mode="HTML")  
    except Exception as e:  
        print(f"❌ Помилка при старті: {e}")  
        bot.send_message([message.chat.id](http://message.chat.id), "Сталася помилка при старті бота.")  
